...
The playback panel also accepts an exact sample position for seek. Seeking preserves the common downstream DSP path and resets no hardware state. It also stores named bookmarks and labeled sample-range annotations in SQLite, linked to the loaded recording session ID; each can seek directly to its saved start position.
