...
function App() {
  ...
  return <main>
    ...
    <div hidden={!visible || view !== "Live"}>
      ...
      <section className="device-panel"><button disabled={recordingBusy || !status?.state.running} onClick={() => void recordingAction(recording ? "DELETE" : "POST")}>{recording ? "Stop recording" : "Start IQ recording"}</button>{recording && <span> {recording.id} · {(recording.bytes_written / 1e6).toFixed(1)} MB · {(recording.elapsed_ms / 1000).toFixed(1)} s · queue {recording.queued_blocks} · drops {recording.dropped_blocks}</span>}<p>SigMF ci8_le · projected {(recording?.projected_bytes_per_second ?? ((status?.state.sample_rate_hz ?? 0) * 2)) / 1e6} MB/s · disk {recording?.available_disk_bytes == null ? "unknown" : `${(recording.available_disk_bytes / 1e9).toFixed(1)} GB free`}. Recording writes raw IQ on a bounded worker.</p></section>
